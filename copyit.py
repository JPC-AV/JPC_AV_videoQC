...
## This script was originally written by Kieran O'Leary and is part of the IFIscripts repo for the Irish Film Institute
## Documentation fo the full suite of scripts is located here: https://github.com/Irish-Film-Institute/IFIscripts
## As of writing, 10/04/2023

## Script was slightly modified by Eddy Colloton in 2023 to combine functions and dependencies originally definied in other files that are part of IFIscripts.

### Usage: python3 copyit.py [source_directory] [destination directory]
## Result: 
# Files are counted, MD5 checksums are generated.
# Files are rsync'd or robocopied (macOS and Window respectively) to destination.
# MD5 checksums of copies are verified. Logs are written to ~/Desktop/script_logs/[name of source directory].log
# Checksum manifests are written to the destination directory.
# Manifest is also written to ~/Desktop/checksum_manifests/old_manifests/
...
def make_desktop_logs_dir():
    desktop_logs_dir = os.path.expanduser("~/Desktop/script_logs")
...
def make_desktop_manifest_dir():
    desktop_manifest_dir = os.path.expanduser("~/Desktop/checksum_manifests")
...
def setup(args_):
    ...
    generate_log(
        log_name_source,
        'eventDetail=copyit.py %s' % get_script_version('copyit.py'))
    ...
    total_input_size = get_folder_size(os.path.abspath(args.source))
    print('Checking if enough space in destination folder')
    free_space = get_free_space(args.destination)
    if total_input_size > free_space:
        print('You do not have enough free space!')
        if args.y:
            go_forth_blindly = 'Y'
        else:
            go_forth_blindly = ask_yes_no('Would you like to continue anyway? Press Y or N')
